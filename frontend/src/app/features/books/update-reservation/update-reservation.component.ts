...
export class UpdateReservationComponent {
  ...
  @Output() updateFinished = new EventEmitter<void>(); // Add output event
  ...
  ngOnInit(): void {
    ...
    this.borrowService.getBookOccupiedDatesByBookIdForUpdatedBorrow(this.bookId, this.borrowId).subscribe({
      ...
      onChange: (selectedDates) => {
        if (selectedDates.length === 2) {
          this.reservationForm.patchValue({
            checkInOut: selectedDates.map(date => this.formatDate(date))
          });
        }
      }
    }) as Instance;
  ...
  createBorrowRequest() {
    ...
    this.borrowService.updateBorrowWhilePending(borrowRequest).subscribe({
      next: (response) => {
        this.updateFinished.emit(); // Emit event to notify parent
